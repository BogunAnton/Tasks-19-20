...
def input_two_mas(array1, array2):
    ...
    :type array1: list 
    :param array2: Исходный массив, представляющий второе большоеое число.
    :type array2: list 
    ...
    array1, array2 = option_handlers.get(int(option), lambda: (array1, array2))() if is_int(option) else (
                print('error') or (array1, array2))
...
def sum_or_difference_arrays(array1, array2):
    ...
    return sum_or_difference_handlers.get(int(sum_or_difference), lambda: 0)() if is_int(sum_or_difference) else 0
...
def get_matrix():
    ...
    matrix = option_handlers.get(int(option), lambda: [])() if is_int(option) else (print("error") or [])
