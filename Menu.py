...
def handle_task_selection(state):
    ...
    choice_task = input()
    return {**state, 'choice_task': int(choice_task), 'task_selected': True} if is_int(choice_task) else state
...
def handle_data_input(state):
    ...
    array1, array2 = data_input_handlers.get(state['choice_task'], lambda x, y: (x, y))(state['array1'],
                                                                                        state['array2'])
    return {**state, 'array1': array1, 'array2': array2, 'data_entered': True} if state['task_selected'] else (
                print("Сначала выполните пункт 1.") or state)
...
def handle_algorithm_execution(state):
    ...
    return {**state, 'final_result': final_result, 'algorithm_executed': True} if state['task_selected'] and state[
        'data_entered'] else (print("Сначала выполните пункты 1 и 2.") or state)
...
def handle_result_display(state):
    ...
    (result_display_handlers.get(state['choice_task'], lambda: None)() if state['task_selected'] and state[
        'data_entered'] and state['algorithm_executed'] else print("Сначала выполните пункты 1, 2, 3."))
    return state
